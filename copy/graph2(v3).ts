// this works finding uses the formular here https://arxiv.org/html/2406.16600v1
// graph.ts
...
export class ArbitrageGraph {
  ...
  private calculateMaxProfit(opportunity: {
    ...
    const { calculateProfit, calculateProfitDerivative } = this.createProfitFunctions(opportunity, pairsInfo);

    // Gradient descent with momentum
    let inputAmount = 1e18; // Initial guess (1 ETH)
    ...
    for (let i = 0; i < numIterations; i++) {
      ...
      const gradient = calculateProfitDerivative(inputAmount);

      // Update velocity with momentum
      velocity = momentum * velocity + learningRate * gradient;
      
      // Update input amount
      const newInputAmount = inputAmount + velocity;
      ...
      // Ensure input amount stays positive
      ...
      // Early stopping if profit is decreasing significantly
      if (i > 0 && profit < maxProfit * 0.5) {
        break;
      }
    }
  ...
  private createProfitFunctions(
    ...
    calculateProfitDerivative: (inputAmount: number) => number;
  } {
    ...
    // Calculate derivative of profit function for optimization
    const calculateProfitDerivative = (inputAmount: number): number => {
      const h = inputAmount * 1e-8; // Small step for numerical derivative
      const profitAtInput = calculateProfit(inputAmount);
      const profitAtInputPlusH = calculateProfit(inputAmount + h);
      
      // Numerical derivative (forward difference)
      return (profitAtInputPlusH - profitAtInput) / h;
    };

    return { calculateProfit, calculateProfitDerivative };
  }
