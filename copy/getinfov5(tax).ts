getinfo.ts==>
    ...
    import taxedPairs from '../taxed.json';
    ...
    export type PairInfo = {
        ...
        factory: string;
        fee: number;
        buyFeeBps: number;     // Added tax info
        sellFeeBps: number;    // Added tax info
    };
     ...
     * unless they are known tax pairs
     */
    async function getPairsInRange(
        ...
        try {
            ...
            // Map pairs and include tax information
            const pairs = pairsData.map(([token0, token1, pairAddress]) => {
                // Check if this is a known tax pair
                const taxInfo = (taxedPairs as Record<string, { buyFeeBps: number; sellFeeBps: number }>)[pairAddress];
                ...
                return {
                    ...
                    fee: factory.fee,
                    buyFeeBps: taxInfo?.buyFeeBps ?? 0,
                    sellFeeBps: taxInfo?.sellFeeBps ?? 0
                };
            });
            ...
            // Filter pairs:
            // 1. Keep pairs that exist in taxedPairs regardless of banned status
            // 2. For other pairs, apply banned token and liquidity pool count filters
            const filteredPairs = pairs.filter(pair => {
                // Always keep known tax pairs
                if ((taxedPairs as Record<string, any>)[pair.pairAddress]) {
                    return true;
                }
    
                // Filter out pairs where either token appears in only one liquidity pool
                if (tokenPoolCount[pair.token0] <= 1 || tokenPoolCount[pair.token1] <= 1) {
                    return false;
                }
    
                // Filter out pairs with banned tokens (unless it's a known tax pair)
